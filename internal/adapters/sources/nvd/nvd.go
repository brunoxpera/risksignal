...
// the local mock sources. The adapter is endpoint-less on purpose — the base
// URL arrives per fetch through the resolved source descriptor
// (FetchInput.Source.Endpoint) — so one adapter instance serves every
// configured NVD source row.
