...
const inventorySnapshot = `-- name: InventorySnapshot :one
...
type InventorySnapshotRow struct {
	AssetsCount            int64
	ComponentsCount        int64
	AssetsMaxUpdatedAt     pgtype.Timestamptz
	ComponentsMaxUpdatedAt pgtype.Timestamptz
	MaxDeactivatedAt       pgtype.Timestamptz
}
...
// table).
func (q *Queries) InventorySnapshot(ctx context.Context) (InventorySnapshotRow, error) {
	row := q.db.QueryRow(ctx, inventorySnapshot)
	var i InventorySnapshotRow
	err := row.Scan(
		&i.AssetsCount,
		&i.ComponentsCount,
		&i.AssetsMaxUpdatedAt,
		&i.ComponentsMaxUpdatedAt,
		&i.MaxDeactivatedAt,
	)
	return i, err
}
