...
const getSignalPriorityFactorSource = `-- name: GetSignalPriorityFactorSource :one
...
type GetSignalPriorityFactorSourceRow struct {
...
func (q *Queries) GetSignalPriorityFactorSource(ctx context.Context, id pgtype.UUID) (GetSignalPriorityFactorSourceRow, error) {
	row := q.db.QueryRow(ctx, getSignalPriorityFactorSource, id)
	var i GetSignalPriorityFactorSourceRow
	err := row.Scan(
		&i.VulnerabilityID,
		&i.Method,
		&i.CveID,
...
const listVulnerabilityFactorEvidence = `-- name: ListVulnerabilityFactorEvidence :many
...
type ListVulnerabilityFactorEvidenceRow struct {
	Type       string
	Value      []byte
...
func (q *Queries) ListVulnerabilityFactorEvidence(ctx context.Context, vulnerabilityID pgtype.UUID) ([]ListVulnerabilityFactorEvidenceRow, error) {
	rows, err := q.db.Query(ctx, listVulnerabilityFactorEvidence, vulnerabilityID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListVulnerabilityFactorEvidenceRow
	for rows.Next() {
		var i ListVulnerabilityFactorEvidenceRow
		if err := rows.Scan(&i.Type, &i.Value, &i.ObservedAt); err != nil {
			return nil, err
		}
