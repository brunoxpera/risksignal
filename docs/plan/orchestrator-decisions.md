...
| Concern | Tool | Notes |
|---|---|---|
| Lint + static analysis | `golangci-lint` | aggregate runner; `govet`, `staticcheck`, `gosec` enabled |
| SAST (security) | `gosec` | runs via golangci-lint |
| Vulnerability scan (Go) | `govulncheck` | database-backed, CI gate |
